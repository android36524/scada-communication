...
public class StorageFactory {
	...
	public static Object parseStorage(String storage) throws IllegalArgumentException {
        if (storage == null) {
            return null;
        }
	...
	public static FaultStorage parseFaultStorage(String storage) throws IllegalArgumentException {
		
		String[] storageInfoArray = storage.split("\\|");
		if (storageInfoArray.length != 5) {
			throw new IllegalArgumentException("存储器格式错误："+storage);
		} else {
			...
			try {
				...
				throw new IllegalArgumentException("存储器格式解析错误："+storage+","+e.getMessage());
			}
		}
	}
	
	public static YXStorage parseYXStorage(String storage) throws IllegalArgumentException {
	...
	public static List<OffLimitsStorage> parseOffLimitsStorages(String storages) throws IllegalArgumentException {
	...
	private static OffLimitsStorage parseOffLimitsStorage(String storage) throws IllegalArgumentException {
		String[] storageInfoArray = storage.split("\\|");
		if (storageInfoArray.length != 5) {
			throw new IllegalArgumentException("存储器格式错误："+storage);
		} else {
