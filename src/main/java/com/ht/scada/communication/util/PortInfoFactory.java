...
public class PortInfoFactory {
	...
	public static DtuInfo parseDtuInfo(String portInfo) throws IllegalArgumentException {
		if (portInfo.startsWith("dtu|")) {
			String[] infoArray = portInfo.substring(4).split(":");
			if (infoArray.length != 4) {
				throw new IllegalArgumentException("DTU信息不完整:"+portInfo);
			}
			...
			try {
				...
				throw new IllegalArgumentException("无法按照DTU格式解析:"+portInfo + "," + e.getMessage());
			}
		} else {
			throw new IllegalArgumentException("无法按照DTU格式解析:"+portInfo);
		}
	}
	 ...
	 */
	public static TcpIpInfo parseTcpIpInfo(String portInfo) throws IllegalArgumentException {
		if (portInfo.startsWith("tcp/ip|")) {
			String[] infoArray = portInfo.substring(7).split(":");
			if (infoArray.length != 2) {
				throw new IllegalArgumentException("TCP信息不完整:"+portInfo);
			}
			...
			try {
				...
				throw new IllegalArgumentException("无法按照TCP格式解析:"+portInfo + "," + e.getMessage());
			}
		} else {
			throw new IllegalArgumentException("无法按照TCP格式解析:"+portInfo);
		}
	}
	 ...
	 * @throws IllegalArgumentException
	 */
	public static SerialInfo parseSerialInfo(String portInfo) throws IllegalArgumentException {
		if (portInfo.startsWith("serial|")) {
			String[] infoArray = portInfo.substring(7).split(":");
			if (infoArray.length != 5) {
				throw new IllegalArgumentException("Serial信息不完整:"+portInfo);
			}
			...
			try {
				...
				throw new IllegalArgumentException("无法按照Serial格式解析:"+portInfo + "," + e.getMessage());
			}
		} else {
			throw new IllegalArgumentException("无法按照Serial格式解析:"+portInfo);
		}
	}
